...
const regionesYComunas = [
  ...
  },
  {
    ...
    comunas: ['Concepción', 'Coronel', 'Chiguayante', 'Florida', 'Hualpén', 'Hualqui', 'Lota', 'Penco', 'San Pedro de la Paz', 'Santa Juana', 'Talcahuano', 'Tomé', 'Cabrero', 'Laja', 'Los Ángeles', 'Mulchén', 'Nacimiento', 'Negrete', 'Quilaco', 'Quilleco', 'San Rosendo', 'Santa Bárbara', 'Tucapel', 'Yumbel', 'Alto Biobío', 'Antuco', 'Curanilahue', 'Arauco', 'Cañete', 'Contulmo', 'Lebu', 'Los Álamos', 'Tirúa']
  },
  {
  ...
  for (const region of regionesYComunas) {
    const createdRegion = await prisma.region.create({
      data: {
        name: region.name,
        comunas: {
          create: region.comunas.map((comuna) => ({ name: comuna }))
        }
      }
    });
